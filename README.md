...
"""
Validates raw log text before sending it to AI/LLM systems.
Performs:
- Format and encoding checks
- PII detection and optional sanitization
- Detection of hardcoded secrets and banned text
- Injection detection (command, SQL, NoSQL, XSS, etc.)
- Suspicious/obfuscated content detection (Base64, hex, payloads)
- Security checks for null bytes and control characters
Returns a detailed ValidationResult with:
- Issues by severity
- Metadata (size, line count, hash ID, timestamp)
- Optionally sanitized log
Ensures logs are safe, clean, and secure for downstream processing.
"""
...
class LogValidator:
    ...
    def validate(self, log_text: str, metadata: Optional[Dict] = None) -> ValidationResult:
        ...
        if self.enable_size_check:
            size_issues = self.validate_size(log_text)
            issues.extend(size_issues)

        # Format validation
        format_issues = self.validate_format(log_text)
        issues.extend(format_issues)

        # Security validation
        security_issues = self.validate_security(log_text)
        issues.extend(security_issues)

        # PII detection
        pii_issues, sanitized_log = self.detect_pii(log_text)
        issues.extend(pii_issues)

        # Injection detection
        injection_issues = self.detect_injections(log_text)
        issues.extend(injection_issues)

        # Suspicious pattern detection
        suspicious_issues = self.detect_suspicious_patterns(log_text)
        issues.extend(suspicious_issues)

        # Banned text detection
        banned_issues = self.detect_banned_text(log_text)
        ...
        is_valid = (
            ...
            if self.strict_mode 
            else len([i for i in issues if i.severity == SeverityLevel.CRITICAL]) == 0
        )
    ...
    def validate_size(self, log_text: str) -> List[ValidationIssue]:
    ...
    def validate_format(self, log_text: str) -> List[ValidationIssue]:
    ...
    def validate_security(self, log_text: str) -> List[ValidationIssue]:
    ...
    def detect_pii(self, log_text: str) -> Tuple[List[ValidationIssue], Optional[str]]:
    ...
    def detect_injections(self, log_text: str) -> List[ValidationIssue]:
    ...
    def detect_banned_text(self, log_text: str) -> List[ValidationIssue]:
    ...
    def detect_suspicious_patterns(self, log_text: str) -> List[ValidationIssue]:
